...
import { ImageIcon, MapPin, Camera, DollarSign } from "lucide-react";
...
function DigitDisplay({ value, suffix = "" }: { value: number; suffix?: string }) {
  ...
  return (
    <div className="flex items-center justify-center gap-[3px]">
      ...
      {suffix && <span className="text-xl sm:text-2xl font-bold text-ocean-400 ml-0.5">{suffix}</span>}
    </div>
  );
}
...
function AnimatedStat({ stat }: { stat: StatItem }) {
  ...
  const Icon = ICON_MAP[stat.icon] || ImageIcon;
  ...
  return (
    <div ref={ref}
      ...
      <div className="mb-3 flex justify-center group-hover:scale-110 transition-transform">
        <div className="w-12 h-12 rounded-xl bg-ocean-500/10 border border-ocean-500/20 flex items-center justify-center">
          <Icon className="w-6 h-6 text-ocean-400" strokeWidth={1.5} />
        </div>
      </div>
